...
impl Cleaner {
    async fn clean(services: Arc<HashMap<ServiceName, LockableService>>) {
        for (service_name, lockable_service) in services.iter() {
            ...
            if n_expired > 0 {
                info!(
                    "Cleared {} expired leases for service {}",
                    n_expired, service_name.0
                )
            }
        }
    }
...
impl AppState {
                    ...
                    .map(|(s_name, service)| {
                        ...
                        let s = Service {
                            expires_in: chrono::Duration::seconds(
                                service.lease_timeout_secs as i64,
                            ),
    ...
    async fn load_persistent_leases(&mut self, leases_path: &Path) -> Result<()> {
        ...
        let persistent_leases: HashMap<ServiceName, Vec<PersistentLease>> =
        ...
        for (service_name, persistent_leases) in persistent_leases.iter() {
            if let Some(lockable_service) = self.services.get(service_name) {
                ...
                let mut cred_hashes = locked_service
                    .available_creds
                    .drain(..)
...
async fn main() -> Result<()> {
    ...
    let app_state = {
        ...
        if let Some(persistent_leases_filename) = &cfg.persistent_leases_filename {
            match a_state
                .load_persistent_leases(Path::new(&persistent_leases_filename))
                .await
            {
    ...
    info!(
        "Starting webserver on {} using path {}",
        cfg.listen_on, cfg.web_path
    );
    ...
    if let Some(ssl_config) = cfg.ssl {
        ...
        builder
            .set_private_key_file(ssl_config.private_key_pem_file, SslFiletype::PEM)
    ...
    if let Some(persistent_leases_filename) = cfg.persistent_leases_filename {
        app_state_for_save
            .persist_leases(Path::new(&persistent_leases_filename))
            .await?
    }
...
async fn get_lease(
    ...
    get_lease: web::Json<GetLeaseRequest>,
    ...
    loop {
        if let Some(lockable_service) = app_state.services.get(&service_name) {
            ...
            if let Some(lease) = locked_service.get_lease(&useragent) {
                ...
                if wait_secs > 10.0 {
                    warn!(
                        "client '{}' had to wait {:.3} seconds to obtain credential for {}",
            ...
            warn!(
                "credential request for unknown service '{}' from client {} received",
...
async fn clear_lease(
    app_state: web::Data<AppState>,
    clear_lease_req: web::Json<ClearLeaseRequest>,
) -> actix_web::Result<HttpResponse> {
    ...
    for (service_name, lockable_service) in app_state.services.iter() {
        ...
        if let Some(release) = locked_service.release(&lease_id) {
            info!(
                "credential for service {} was in use for {:.3} secs by client '{}'",
                service_name.0,
                release.duration.num_milliseconds() as f64 / 1000.0,
                release.client_name
            );
            break;
        }
